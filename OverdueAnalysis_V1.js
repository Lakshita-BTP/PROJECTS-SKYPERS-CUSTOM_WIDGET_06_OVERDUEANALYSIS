...
========================= */
    ...
    `;
      ...
      try {
        ...
        const data = this._myDataBinding.data.map((row) => ({
          ...
          value: Number(row[measure].raw) || 0,
        }));
    ...
    =================================== */
        ...
        if (data.length >= 3) {
          gt30 = data.slice(2).reduce((sum, item) => sum + item.value, 0);
        }

        if (data.length >= 4) {
          gt60 = data.slice(3).reduce((sum, item) => sum + item.value, 0);
        }
